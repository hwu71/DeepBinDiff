...
def optimize_edit_paths(
    ...
    def form_node_subst_matrix(nodes_u, nodes_v,node_subst_cost=None):
       array=[]
       if node_subst_cost:
           for u in nodes_u:
               for v in nodes_v:
                   array.append(node_subst_cost(u,v))
           array = np.array(array)
           return array.reshape(len(nodes_u), len(nodes_v))
       else:
           return np.zeros((len(nodes_u), len(nodes_v)))
 
    def form_edge_subst_matrix(edges_g, edges_h, edge_subst_cost=None):
       array=[]
       if edge_subst_cost:
           for g in edges_g:
               for h in edges_h:
                   array.append(edge_subst_cost(g,h))
           array = np.array(array)
           return array.reshape(len(edges_g), len(edges_h))
       else:
           return np.zeros((len(edges_g), len(edges_h)))
    ...
    C[0:m, 0:n] = form_node_subst_matrix(pending_u, pending_v,node_subst_cost=node_subst_cost)
    if roots:
        initial_cost = node_subst_cost(root_u, root_v)
    elif node_match:
        C[0:m, 0:n] = np.array(
            [
    ...
    if edge_subst_cost:
        ...
        '''
        C[0:m, 0:n] = form_edge_subst_matrix(pending_g, pending_h, edge_subst_cost=edge_subst_cost)
    elif edge_match:
        C[0:m, 0:n] = np.array(
            [
    ...
    if edge_del_cost:
        ...
        del_costs = [edge_del_cost(g) for g in pending_g]
    else:
        del_costs = [1] * len(pending_g)
    # assert not m or min(del_costs) >= 0
    if edge_ins_cost:
        #ins_costs = [edge_ins_cost(G2.edges[h]) for h in pending_h]
        ins_costs = [edge_ins_cost(h) for h in pending_h]
    else:
